...
'''FOR EACH DIGIT'''
# find digit
indlt = sess.run(indLT)
subxw = sess.run(subXW)
for index in range(5):
    key = np.where(indlt == index)
    keyList = [subxw[i] for i in key]
    truelist = np.where(keyList[0] == 0)
    truenum = len(truelist[0])
    allnum = len(keyList[0])
    ratio = truenum/allnum
    errRatio = 1-ratio
    print("for", index, "ratio is :", ratio)
    print("ERROR rate is:", errRatio)
#
